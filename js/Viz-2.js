	...
	//----------- Start of Viz 22------------------------------------- Start of Viz 22------------------------------------
		...
		xScale4.domain(data.map(function(d){return d.WorkspaceClassification}));

		
		yScale4.domain([-30,300]).nice();
	  ...
	  svg4.append("g")			
		...
		var xCord = data.map(function(d){ return xScale4(d.WorkspaceClassification) + 65});
		var shapeCircle = d3.symbol().size(150).type(d3.symbolCircle);
		var shapeTriangle = d3.symbol().size(150).type(d3.symbolTriangle);
	    ...
	    d3.selectAll('.point4').on("mouseover", function(d){
			...
			cardText4.html("<p><b> Peak Force: </b>" + d.PeakForceNewton + "<br>" + "<b> Workspace: </b>" + d.WorkspaceClassification + "<br>" + "<b> DoF: </b>" + d.NumberOfDoF + "</p>");
